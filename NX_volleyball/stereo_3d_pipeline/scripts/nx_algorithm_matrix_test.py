...
def write_reports(out_dir: Path, rows: list[dict[str, str]], duration_sec: int, project: Path) -> None:
    ...
    lines = [
        ...
        "- `algo_*` is the profiler stage selected for this case; `async_worker_*` is full async Stage2.",
        ...
        "",
        "| case | diagnosis | status | fps | algo_stage | algo avg/max | worker avg/max | over_deadline | stale/expired | queue_drop | candidate_valid/frames | rate | median/MAD | support | accepted | frame_cb_skip | host_gray | debug_dirs | note | last error/warn |",
        "|---|---|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---|",
    ]
    for row in rows:
        stale_expired = (
        ...
        values = [
            ...
            row.get("fps_last", ""),
            row.get("algo_stage", ""),
            f'{row.get("algo_avg_ms", "")}/{row.get("algo_max_ms", "")}',
            f'{row.get("async_worker_avg_ms", "")}/{row.get("async_worker_max_ms", "")}',
