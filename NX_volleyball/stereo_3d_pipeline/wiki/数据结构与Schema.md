...
`z_stereo` 和 `z` 也是兼容字段，不是新增候选方法。当前实时管线用 `buildDepthCandidateObservations()` 收集候选，并由 `selectFirstUsableDepthCandidate()` 取第一个可用候选写入 `z_stereo/z/raw_z/stereo_depth_source`。训练可靠性模型时应读取各个原始 `z_*` 候选字段；`z_stereo/z` 只作为旧在线 baseline 或诊断字段。
