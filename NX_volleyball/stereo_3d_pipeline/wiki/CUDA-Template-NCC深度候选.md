...
本页记录 `z_roi_cuda_template_match` 当前实现。它已经不再默认调用 `cv::cuda::TemplateMatching`，而是使用项目内自研 CUDA 小窗口 Template/NCC kernel；OpenCV CUDA 版本仅作为 A/B baseline 保留。
...
- 该字段不参与 legacy `z_stereo/obj.z` 选择；训练应直接读取原始 `z_roi_cuda_template_match`。
...
- 该方法适合作为 P2 训练候选或条件触发候选，不应直接提升为 legacy 在线主深度。
