...
| VPI Template/Stereo | diagnostic `0/645`、`0/632` 有效 | 真实 VPI CUDA diagnostic 后端已运行，但当前参数/gate 无有效深度 |
| VPI Harris-LK/ORB | diagnostic `0/636`、`0/637` 有效 | 当前项目只是 availability stub，真实 VPI 后端未实现 |
| CUDA-SIFT / Fixstars libSGM | diagnostic `0/629-635` 有效 | 当前项目只是 availability stub，NX 未引入第三方库 |
...
| CUDA ring/edge profile | 自研 CUDA 多环边缘/灰度 profile，小范围极线搜索 | `z_roi_ring_edge_profile` | `Stage2_CudaRingEdgeProfileMatch` / `Stage2_P2FeatureJobDiagnosticCudaRingEdgeProfile` | 已接入默认关闭字段和 diagnostic-only case，待有球 NX isolated 实测 |
