...
这些 artifact 来自后端返回的 `debug_matches` / peak，不是 realtime status zoom。当前已验证 OpenCV CUDA ORB、OpenCV CUDA Template、OpenCV CUDA StereoSGM、OpenCV CUDA GFTT/LK、CUDA Hough circle、VPI Template 和 VPI ORB 可以输出算法级样张；仍不能把带 PNG 写盘的 run 当作 FPS 准入。
...
`codex_p2_verify_20260704_104947` 是无 debug 性能复核；`codex_p2_artifact_debug_20260704_105356` 和 11:08 final artifact run 只用于算法级 artifact 抽样。样张已复制到 `wiki/assets/p2_20260704_artifacts/` 和 `wiki/assets/p2_20260704_final/`，详见 [P2算法效果与可视化审查](P2算法效果与可视化审查.md)。
