...
| BRISK | OpenCV CPU | `0.2` | `24.78/41.81ms` | `252` | `0/2` | - | CPU 调试项, 不适合实时 |
| AKAZE | OpenCV CPU | `91.9` | `8.48/17.98ms` | `55` | `0/536` | - | CPU 调试项, 当前无有效深度 |
| SIFT | OpenCV CPU | `0.2` | `21.59/55.30ms` | `289` | `0/2` | - | CPU 调试项, 不适合实时 |
...
| SuperPoint 224/top128 | TensorRT extractor + CPU 后处理 | `8.6` | `14.60/31.65ms` | `332` | `5/114` | `3.4992/0.0013m` | engine 可跑, 但耗时和有效率都不达标 |
...
- 默认实时配置继续用几何候选；`roi_center_patch` 和 `roi_subpixel` 可作为质量优先 A/B 项，但它们和默认几何有约 `7-10cm` 的系统深度差，需要用真实轨迹或标定片段评估偏置。
- `corner/texture/binary` sparse-lite 可用于诊断，不应写成 OpenCV ORB/BRISK/AKAZE/SIFT 的真实 GPU 实现。
- OpenCV CUDA ORB 是真实 GPU 算法；strict gate 下有效率只有 `27/399`，relaxed 后到 `249/389`，但 worker 平均仍约 `11ms`、尾延迟约 `45-50ms`，不适合默认 100fps。
- BRISK/AKAZE/SIFT 当前是 CPU OpenCV 路径，只适合 debug/A-B。
- XFeat 是当前唯一低于 10ms 平均耗时的神经特征 TensorRT 路径，但 strict 有效率 `20/583`，relaxed 为 `0/575`，不能作为默认深度源。
- SuperPoint extractor engine 可运行，但平均 `14-15ms` 且有效率很低，C++ 后处理和 gate 后结果不达标。
