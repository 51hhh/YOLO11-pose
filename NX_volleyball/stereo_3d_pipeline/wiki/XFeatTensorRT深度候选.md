...
本页记录 XFeat TensorRT 神经特征深度候选。该路径已经接入主 CSV 字段 `z_roi_neural_feature`，也能通过 P2 diagnostic sidecar 输出 `mode=neural_feature` 的真实左右匹配 artifact；但当前默认关闭，不属于正式 100fps 采集集合。

## 当前结论

XFeat 目前不能作为正式深度测量方案。主要原因不是模型推理慢，而是 inline 主路径会把 TensorRT、GPU postprocess、D2H 和同步放进 `Stage2_AsyncRoiWorker` deadline；关键点质量也还不足。
...
| Diagnostic sidecar | `*.p2_diagnostic.csv` 中 `mode=neural_feature` |
