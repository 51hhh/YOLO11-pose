...
| OpenCV CUDA ORB | gray GPU | `cv::cuda::ORB` + CUDA matcher | 主路径无 CPU 图像；GPU 无效后可走 CPU ORB fallback | 真 GPU ORB，但当前场景 0 valid 且同步慢 |
| OpenCV BRISK/AKAZE/SIFT | gray pinned host | CPU OpenCV | 必需 CPU 图像 | 当前 NX 无 true CUDA 后端，默认关闭 |
| XFeat extractor-only | gray 或 BGR GPU ROI | TensorRT extractor | 不需要 CPU 图像；网络输出 D2H 后做 NMS、descriptor 采样和 mutual-NN | 后处理仍 CPU，不是完整 GPU 常驻匹配 |
| ALIKED/SuperPoint direct extractor | gray 或 BGR GPU ROI | TensorRT extractor | 不需要 CPU 图像；输出 D2H 后做 descriptor mutual-NN/gate | 需要真实 engine 输出 `keypoints/descriptors/scores` |
| TensorRT split matcher | gray 或 BGR GPU ROI | TensorRT extractor + TensorRT matcher | 不需要 CPU 图像；当前中间特征会 D2H/H2D，最终 gate 在 CPU | 仅 `use_lightglue=true` 时启用；matcher 只支持固定 keypoint/descriptor 输入和 `matches`/`matches0` 输出 schema |
| SuperPoint+LightGlue fused | gray 或 BGR GPU ROI | TensorRT fused matcher | 不需要 CPU 图像；输出点对 D2H 后 CPU gate | 需要 fused engine 输出 `[N,4]` 或 `[N,5]` |
...
- pinned host gray 只在当帧确实可能进入 CPU OpenCV 描述子或 CPU fallback 时复制；空检测不复制，单侧检测只有 fallback 可能运行时复制。
