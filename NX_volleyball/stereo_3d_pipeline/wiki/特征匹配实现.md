...
| SuperPoint+LightGlue | Python probe；fused 入口有；direct extractor fallback 有 | 未 NX 验证 | 离线可匹配 | LightGlue split matcher 仍需真实 engine 闭环 |
...
- split LightGlue/matcher engine 的多输入运行时还不是完整生产路径；推荐优先导出 fused `[N,4/5]` 点对输出 engine。
