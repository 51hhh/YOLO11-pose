...
- `test_logs/volleyball_raw_pair_latest/offline_feature_eval_fixed_20260703/summary.csv`
...
| BRISK | OpenCV CPU debug/A-B | 无 true CUDA 后端 | CPU 单帧点数少；历史 lite 在线 0 有效 | 不能写成真实 GPU 实时方法 |
| AKAZE | OpenCV CPU debug/A-B | 无 true CUDA 后端 | CPU 单帧点数少；历史 lite 在线 0 有效 | 不能写成真实 GPU 实时方法 |
| SIFT | OpenCV CPU/离线 | 无 true CUDA 后端 | CPU 单帧点数极少；历史 lite 在线 0 有效 | 不能写成真实 GPU 实时方法 |
...
- VPI 官方文档有 ORB feature detector/descriptor API，是可评估的另一个真实 ORB 后端候选；本项目尚未实现 VPI ORB，不能把它计入当前实时能力。
...
| `roi_center_patch`, `roi_subpixel`, `multi_point` | gray GPU | CUDA + 少量 CPU gate | 无 CPU 图像 | `roi_subpixel/multi_point` 同步实测不守 100fps |
| `corner/texture/binary` sparse-lite | gray GPU | 自研 CUDA sparse-lite | 无 CPU 图像 | 不是 OpenCV 描述子算法，当前 gate 下 0 valid |
| 彩色 IoU/edge patch | BGR GPU | CUDA | 无 CPU 图像 | 需要 BGR 管线和 async BGR snapshot |
| OpenCV CUDA ORB | gray GPU | `cv::cuda::ORB` + CUDA matcher | 不需要 CPU 图像；keypoints/matches 元数据会回 CPU 做 gate；`gpu_candidate_refine=true` 时不为 CPU ORB fallback 预先 D2H | 真 GPU ORB，但当前场景 0 valid 且同步慢 |
...
- 初始 ROI 中心视差: `431.00px`, 初始深度: `3.4542m`
- 输出资产: `wiki/assets/feature_matching/volleyball_raw_pair_latest_20260703/`

20260703 修复点: 旧 wiki 图里左 ROI 漂到背景，原因是离线自动 ROI 在 YOLO/IoU 退化场景下直接拿颜色碎片 bbox 配对，正确排球碎片会被 size gate 拒绝，fallback 又选到左侧背景色块。现在离线 probe 先把颜色碎片合并成球尺寸粗 ROI，再做左右 pair gate 和 Hough refine，并加入物理半径一致性打分。
...
- `iou_region_color_patch` 在当前单帧严格通过，是后续连续彩色 PNG clip 的优先候选；是否进实时默认配置仍要看连续帧稳定性。
...
100Hz 同步帧
  ...
  -> 可选 true OpenCV CUDA ORB、CPU OpenCV BRISK/AKAZE/SIFT、TensorRT XFeat/ALIKED/SuperPoint/LightGlue
...
- ALIKED/SuperPoint direct extractor、split LightGlue matcher 和 fused matcher 都已有 C++ TensorRT 入口；它们是否可用取决于真实 engine 的固定 shape 和 tensor name/schema 是否被 runtime 识别，当前尚未在 NX 上完成闭环验证。
...
- 当前 OpenCV/VPI 资料未暴露 BRISK、AKAZE、SIFT 的等价 CUDA 后端；这些字段只能走 CPU OpenCV 或历史 lite 对照，不能写成真实 GPU 实时实现。
- VPI 文档有 ORB feature detector/descriptor API，属于可评估的真实 ORB 后端候选；本项目尚未实现 VPI ORB 后端，也没有 NX 实测矩阵。
