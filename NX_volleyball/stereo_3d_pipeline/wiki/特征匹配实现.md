...
- BGR GPU 只在当帧左右目都有检测，且彩色候选或 3/6 通道神经 engine 可能运行时复制。
- pinned host gray 只在当帧确实可能进入 CPU OpenCV BRISK/AKAZE/SIFT 或 CPU fallback 时复制；空检测不复制，单侧检测只有 fallback 可能运行时复制。
