...
- 新增算法级 artifact: 11:08 final artifact run 生成 `65` 张真实 PNG；OpenCV CUDA ORB/Template/SGM/GFTT-LK、CUDA Hough circle 和 VPI Template 有样张。13:02 inline 重测补齐 color/color-edge/XFeat 真实匹配图；13:06/13:14 dense 重测补齐 BM/SGM/VPI Stereo/libSGM 的 32x32 disparity patch，VPI Stereo 额外有 confidence patch。VPI ORB 在 10:53 debug run 有少量点对样张，但 11:08 final 段未复现。ring-edge、VPI Harris/LK、CUDA-SIFT 本轮仍无有效 artifact。
...
|---:|---|---|---|---|---|---|
| 1 | `patch_iou_color_edge_wide_search` | `z_roi_patch_iou_color_edge`，自研 CUDA BGR edge/color patch | 10:49 targeted `100.1fps`；algo `0.85/1.17/4.22ms`；worker `1.13/1.48/7.06ms` | `654/654`，median/MAD `3.3334/0.0000m` | 已提升为主管线 P1 主 CSV 候选；13:02 inline retest `640/643`，需联合长测 | artifact<br><a href="assets/p2_20260704_inline/patch_iou_color_edge.png"><img src="assets/p2_20260704_inline/patch_iou_color_edge.png" width="220"></a> |
| 2 | `iou_region_color_patch_wide_search` | `z_roi_iou_region_color_patch`，自研 CUDA BGR patch | 10:49 targeted `100.0fps`；algo `0.85/1.09/4.10ms`；worker `1.14/1.39/7.24ms` | `647/647`，median/MAD `3.3250/0.0000m` | 已提升为主管线 P1 主 CSV 候选；13:02 inline retest `615/621`，需联合长测 | artifact<br><a href="assets/p2_20260704_inline/iou_region_color_patch.png"><img src="assets/p2_20260704_inline/iou_region_color_patch.png" width="220"></a> |
...
| 5 | OpenCV CUDA StereoSGM patch9 | `z_roi_cuda_stereo_sgm`，`cv::cuda::StereoSGM` | 10:49 targeted `99.0fps`；algo `2.30/3.54/10.70ms`；worker `2.38/3.70/10.87ms` | `139/624`，median/MAD `3.3136/0.0031m`；13:14 diagnostic `3/549` | 已提升为主管线 P1 主 CSV 候选；有效率低且 max 过 10ms，只用于训练观察 | artifact<br><a href="assets/p2_20260704_dense/opencv_cuda_stereo_sgm_valid_patch.png"><img src="assets/p2_20260704_dense/opencv_cuda_stereo_sgm_valid_patch.png" width="220"></a> |
...
| 9 | XFeat TensorRT | `z_roi_neural_feature`，extractor-only TensorRT + 当前 C++ 后处理 | 10:49 targeted 128/top32 `93.2fps`；algo `4.14/5.35/11.88ms`；worker `4.27/5.57/12.81ms` | `317/579`，median/MAD `3.3155/0.0015m`；13:02 retest `67/586` | 有效率明显改善，但 FPS/长尾仍不准入；已有真实 neural match overlay | artifact<br><a href="assets/p2_20260704_inline/neural_xfeat.png"><img src="assets/p2_20260704_inline/neural_xfeat.png" width="220"></a> |
