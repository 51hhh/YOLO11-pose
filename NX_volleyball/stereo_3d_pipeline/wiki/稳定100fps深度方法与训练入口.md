...
| OpenCV CUDA ORB | `z_roi_orb_points` | true `cv::cuda::ORB` + CUDA BF；fast48 为 `84.6fps`、`21/198` 有效，wide_y 为 `94.5fps`、`51/397` 有效但有 `6` 次超 deadline | 不进入默认；若继续做，只能 selective/diagnostic |
| OpenCV CUDA TemplateMatching | `z_roi_cuda_template_match` | inline `101.7fps` 但 `0/235` 有效且长尾 `41.38ms`；patch9 + CUDA peak reduction 为 `98.8fps`、`0/414` 有效、max `43.30ms`；diagnostic CSV 可记录逐帧结果 | 不进入默认；只保留诊断/参数试验 |
| OpenCV CUDA StereoBM | `z_roi_cuda_stereo_bm` | 默认/patch9 均为 `0` 有效；patch9 为 `85.4fps`、`0/409` 有效 | 不进入默认；优先改 GPU 内聚合/gate |
| OpenCV CUDA StereoSGM | `z_roi_cuda_stereo_sgm` | 默认/patch9 均为 `0` 有效；patch9 为 `101.1fps`、`0/436` 有效，仍有长尾 | 不进入默认；优先改 GPU 内聚合/gate |
| CUDA ring/edge profile | `z_roi_ring_edge_profile` | 自研 CUDA 多环边缘/灰度 profile；字段、CSV、训练读取和 diagnostic-only case 已接入，待有球 NX isolated 实测 | 不进入默认；先看 diagnostic/isolated 有效率和尾延迟 |
| XFeat TensorRT | `z_roi_neural_feature` | 96 engine trtexec `0.33ms`，但 realtime 96/top32 仅 `6/229` 有效、96/top64 为 `0/248`；追加测试中 128/top32 为 `94.7fps`、`24/375` 有效，128/top96 为 `96.4fps`、`2/402` 有效，160/top64 为 `87.9fps`、`6/398` 有效 | 后处理/NMS/匹配迁到 GPU 前不准入 |
...
| 方向 | 建议形态 | 当前判断 |
|---|---|---|
| ROI ring/edge profile matcher | 复用 circle/edge/radial 信息，沿球边界或径向采样，做小范围极线匹配 | 初版自研 CUDA kernel 和字段已接入；待有球 isolated/diagnostic 实测后决定是否保留 |
