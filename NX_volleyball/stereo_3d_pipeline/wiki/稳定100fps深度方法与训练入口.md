...
| VPI CUDA Template Matching | `roi_vpi_template_match`，VPI CUDA stream wrapper + NCC score 小 ROI | 真实 VPI CUDA diagnostic 后端已跑；本轮 `99.4fps`、`0/645` 有效，不准入 |
| VPI CUDA Stereo Disparity | `roi_vpi_stereo_disparity`，小 ROI / `64` local disparity + confidence map | 真实 VPI CUDA diagnostic 后端已跑；本轮 `100.1fps`、`0/632` 有效，不准入 |
