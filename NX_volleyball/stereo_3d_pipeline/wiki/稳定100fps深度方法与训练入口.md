...
2026-07-04 联合复测显示，`full12_default` 只有 `6.7fps`，`no_sgm_no_vpi` 可回到 `100.3fps`，但 worker `avg/p95/max=9.09/9.72/15.04ms`，仍有 `8` 次 stale 和 `62` 次 over deadline。因此当前测试配置改为: 去 SGM、去 VPI、去两个已确认错配的彩色 IoU/edge 候选；在这个基础上保留 OpenCV CUDA GFTT/LK diagnostic sidecar，并接入但默认关闭 XFeat TensorRT 主 CSV 候选。
...
当前 100fps 默认态一共是 `12` 个主 CSV direct/fallback 深度字段 + `1` 个 diagnostic sidecar 字段: P0 direct 字段 `9` 个，包括 `z_bbox_center`、`z_bbox_left_edge`、`z_bbox_right_edge`、`z_circle_center`、`z_circle_left_edge`、`z_circle_right_edge`、`z_roi_edge_centroid`、`z_roi_radial_center`、`z_roi_edge_pair_center`；P1 主 CSV 字段 `2` 个，为 `z_roi_multi_point`、`z_roi_center_patch`；单侧退化字段 `z_fallback_epipolar`；diagnostic sidecar 字段 `z_roi_opencv_cuda_gftt_lk`。按算法族粗分是 bbox、circle、ROI 几何、multi-point、center patch、epipolar fallback、GFTT/LK 共 `7` 类。`z_roi_neural_feature` 的 XFeat TensorRT 路径已接入但默认关闭。
...
判断优先级: `codex_p2_verify_20260704_104947` 是 07-04 P2 targeted 复核依据；`cuda_template_custom_tiebreak_20260705_074117` 是 07-05 自研 CUDA Template/NCC 依据；`codex_p2_full_20260704_083048` 保留为广覆盖矩阵历史。debug / artifact run 只用于证明算法级点对或峰值位置，不用于 FPS 准入。2026-07-04 联合复测后，`patch_iou_color_edge_wide_search`、`iou_region_color_patch_wide_search` 因 artifact 显示系统性错配而退出默认配置；OpenCV CUDA StereoSGM、VPI Template、VPI ORB 因联合运行长尾或有效率问题退出默认配置。2026-07-05 自研 CUDA Template/NCC 已通过单算法 isolated 准入，但仍默认关闭，等待后续联合配置复测。当前默认只保留 OpenCV CUDA GFTT/LK sidecar；XFeat TensorRT 主字段已接入但默认关闭，需低频化/条件触发后再考虑加入正式采集。
...
| 6 | 自研 CUDA Template/NCC | `z_roi_cuda_template_match`，gray patch NCC + GPU peak reduce | 2026-07-05 A/B `100.1fps`；algo `0.30/1.06/4.89ms`，p99 `1.20ms` | `1372/1374`，median/MAD `3.4831/0.0019m` | 满足单算法 100fps isolated 准入；仍是单点 `support=1` P2 候选，默认关闭，不参与 legacy `z_stereo` | artifact<br><a href="assets/cuda_template_ncc_20260705/frame_000002_valid.png"><img src="assets/cuda_template_ncc_20260705/frame_000002_valid.png" width="220"></a> |
...
| 9 | XFeat TensorRT | `z_roi_neural_feature`，extractor-only TensorRT + 当前 C++ 后处理 | 10:49 targeted 128/top32 `93.2fps`；algo `4.14/5.35/11.88ms`；worker `4.27/5.57/12.81ms`；14:30 xfeat-only `98.3fps` | `317/579`，median/MAD `3.3155/0.0015m`；14:30 xfeat-only `14/1052` | 真实 TensorRT 路径已接入但默认关闭；每帧主路径不满足 100fps，只能单独 A/B 或后续低频化/条件触发 | artifact<br><a href="assets/p2_20260704_inline/neural_xfeat.png"><img src="assets/p2_20260704_inline/neural_xfeat.png" width="220"></a> |
...
默认 `config/pipeline_dual_yolo_roi.yaml` 下，P0、`z_roi_multi_point` 和 `z_roi_center_patch` 会写入主 CSV，OpenCV CUDA GFTT/LK 写入 sidecar。XFeat TensorRT、OpenCV ORB、CUDA Template/NCC、OpenCV BM/SGM、VPI、ring-edge、CPU fallback 等其余 P2 默认关闭。实际允许参与 legacy `z_stereo` 兼容选择的候选为:
...
其中 `z_fallback_epipolar` 只在单侧漏检且极线 fallback 有效时排在该槽位；正常左右 YOLO 配对时同一槽位通常是 `z_circle_center`。`z_fallback` 仍作为兼容汇总字段写出。`z_roi_multi_point`、`z_roi_center_patch` 和 GFTT/LK sidecar 会继续写入原始候选，但不会写入 legacy `z_stereo/obj.z`。模型训练和评估必须直接读取原始 P0/P1 字段。
...
2. 再加入 P1 的 `roi_multi_point`、`roi_center_patch` 和 GFTT/LK sidecar，判断各自系统差是否稳定可校正。XFeat 只能单独 A/B，不能混入正式 100fps 采集。
...
当前训练采集字段冻结为上述 P0+P1 集合；GFTT/LK 写 diagnostic sidecar 并由 `trajectory_fusion/dataset.py` 合并。XFeat TensorRT 已接入但默认关闭。彩色 IoU/edge、SGM、VPI 已从默认配置移除。P2 新算法可以有默认关闭字段和矩阵 case，但没有 NX isolated 和联合结果前不得加入主管线采集配置。
