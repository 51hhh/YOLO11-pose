...
默认 `config/pipeline_dual_yolo_roi.yaml` 下，P0、`z_roi_multi_point` 和 `z_roi_center_patch` 会写入主 CSV，OpenCV CUDA GFTT/LK 写入 sidecar。XFeat TensorRT、OpenCV ORB/Template/BM/SGM、VPI、ring-edge、CPU fallback 等其余 P2 默认关闭。实际允许参与 legacy `z_stereo` 兼容选择的候选为:
