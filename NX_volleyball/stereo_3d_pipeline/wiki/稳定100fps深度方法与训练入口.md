...
原来写在更低优先级里的方法现在按“是否有可测试的 GPU 化路线”重分到 P2。历史失败结果只说明当时参数或后处理不达标，不再作为永久否定结论。

### P2 已接入实时字段，待参数复测

| 方法 | 字段/输出 | 当前状态 | 明日测试重点 |
|---|---|---|---|
| `roi_iou_region_color_patch` | `z_roi_iou_region_color_patch` | 自研 CUDA BGR color patch，历史 strict/relaxed 都 `0` 有效 | 复测离线推荐参数 `patch_radius=9`, `min_confidence=0.54`, `y_tolerance=1px`，再测宽搜索参数 |
| `roi_patch_iou_color_edge` | `z_roi_patch_iou_color_edge` | 自研 CUDA BGR edge/color patch，开销合格但 gate 后无有效深度 | 同上，确认是颜色/边缘响应问题还是在线 gate/初始视差问题 |
| OpenCV CUDA ORB | `z_roi_orb_points` | true `cv::cuda::ORB` + CUDA BF matcher；relaxed 有效率可上升，但平均约 `11ms` 且尾延迟大 | 复测 `fast48` 小 ROI/少点数配置和宽 y gate 配置，判断是否是参数导致超时 |
| OpenCV CUDA TemplateMatching | `z_roi_cuda_template_match` | 已接入，默认关闭，不参与 legacy 输出 | strict、relaxed、`patch9` 三档单测 |
| OpenCV CUDA StereoBM | `z_roi_cuda_stereo_bm` | 已接入局部 ROI dense disparity，默认关闭 | strict、relaxed、`patch9` 三档单测，关注系统偏置和 support |
| OpenCV CUDA StereoSGM | `z_roi_cuda_stereo_sgm` | 已接入局部 ROI dense disparity，默认关闭 | strict、relaxed、`patch9` 三档单测，关注是否比 BM 更稳 |
| XFeat TensorRT | `z_roi_neural_feature` | extractor engine 已可跑；当前 CPU 后处理/匹配有效率低 | 测 `96/128/160/224` ROI 和 `top32/top64/top96/top128`，优先找 10ms 内有效率最高组合 |
| SuperPoint TensorRT extractor | `z_roi_neural_feature` | fixed extractor engine 可跑，但 224/top128 超预算 | 测 `128/top64`, `160/top64`, `224/top64`，判断是否 top-k/ROI 尺寸导致超时 |
