...
这些方法已进入 `config/pipeline_dual_yolo_roi.yaml` 和 `config/pipeline_record_p0p1.yaml` 的主管线采集集合，但不能直接视为稳定主路径。训练读取主 CSV 原始候选字段，用模型学习其偏置和可靠性；diagnostic sidecar 只在专项调试时开启。legacy `z_stereo/obj.z` 仍只允许 P0 几何/bbox 和退化 fallback。
...
其中 `z_fallback_epipolar` 只在单侧漏检且极线 fallback 有效时排在该槽位；正常左右 YOLO 配对时同一槽位通常是 `z_circle_center`。`z_fallback` 仍作为兼容汇总字段写出。`z_roi_multi_point`、`z_roi_center_patch` 和 NCC/XFeat/SuperPoint inline 候选会继续写入原始候选，但不会写入 legacy `z_stereo/obj.z`。模型训练和评估必须直接读取原始 P0/P1/P2-inline 字段。
