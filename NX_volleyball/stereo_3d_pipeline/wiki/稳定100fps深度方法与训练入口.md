...
- 基线采集: 只开 P0，验证 100fps、水印、几何一致性和真实距离 bias。
- 训练采集: P0 + `roi_subpixel`，验证系统偏差是否可由模型稳定校正。
- `roi_center_patch` 仅在需要研究 patch 偏置时开启，不作为常规训练字段依赖。
...
当前 C++ 里仍有 legacy `z_stereo` / `z` 输出，用于兼容 ROS、日志和旧轨迹预测。实时管线会通过 `buildDepthCandidateObservations()` 组装候选，再由 `selectFirstUsableDepthCandidate()` 选择第一个可用候选写入 `z_stereo`、`obj.z`、`raw_z` 和 `stereo_depth_source`。训练模型时不要把 `z_stereo` 或 `z` 当作一个独立深度方法，也不要把它当作标签；它们只能作为 legacy baseline/诊断字段。`trajectory_fusion/dataset.py` 中的 `METHOD_COLUMNS` 已只包含原始候选方法字段。

当前 `first usable` 的完整候选顺序以 `src/stereo/depth_candidate_builder.cpp` 为准:
...
默认 `config/pipeline_dual_yolo_roi.yaml` 下，`roi_subpixel`、patch、颜色、神经、OpenCV 特征和 CPU fallback 默认关闭，因此排在前面的候选多数是无效/未运行状态。默认配置中实际会参与 `z_stereo` 兼容选择的优先顺序通常是:
...
其中 `z_fallback` 只在单侧漏检或 fallback 路径有效时排在该槽位；正常左右 YOLO 配对时同一槽位通常是 `z_circle_center`。这个顺序只解释 legacy `z_stereo` 的兼容输出，不代表训练模型的深度决策顺序。
