...
2026-07-04 有球复测后，只有 `patch_iou_color_edge_wide_search` 明确满足 100fps + 全有效；`iou_region_color_patch_wide_search` 全有效但 20s 长测为 `99.2fps`，暂作为备选长测。它们仍不进入默认 P0+P1，也不参与 legacy 在线主深度。
...
|---|---|---|---|
| `roi_patch_iou_color_edge_wide_search` | `z_roi_patch_iou_color_edge` | 20s 长测 `100.7fps`、`1861/1861` 有效，worker p95 `1.77ms`，当前最稳 P2 | 可做 P2 训练候选长测和真实距离标定 |
| `roi_iou_region_color_patch_wide_search` | `z_roi_iou_region_color_patch` | 20s 长测 `99.2fps`、`1826/1826` 有效，worker p95 `2.04ms` | 备选训练候选；需更长测试确认是否可稳定 100fps |
| `roi_patch_iou_color_edge` / `roi_iou_region_color_patch` base | 同上 | 20s 长测几乎全有效，但 worker max `16.00ms` / `13.60ms`，不如 wide_search | 不作为首选；保留参数对照 |
| OpenCV CUDA StereoSGM patch9 | `z_roi_cuda_stereo_sgm` | 20s 长测 `98.5fps`、`939/1833` 有效，worker max `10.38ms` | 不进入默认；只有代码/聚合优化后再复测 |
| OpenCV CUDA ORB | `z_roi_orb_points` | true `cv::cuda::ORB` + CUDA BF；本轮 fast48 为 `92.3fps`、`2/559` 有效，wide_y 为 `91.7fps`、`134/566` 有效 | 不进入默认；每帧运行不准入 |
| OpenCV CUDA TemplateMatching | `z_roi_cuda_template_match` | patch9 为 `96.0fps`、`0/633` 有效、max `40.42ms` | 不进入默认；只保留诊断/参数试验 |
| OpenCV CUDA StereoBM | `z_roi_cuda_stereo_bm` | patch9 为 `95.2fps`、`0/635` 有效 | 不进入默认；优先改 GPU 内聚合/gate |
| CUDA ring/edge profile | `z_roi_ring_edge_profile` | diagnostic `100.0fps`、`0/652` 有效；support=0/low confidence | 不进入默认；需要算法重写或改采样/gate 后再测 |
| OpenCV CUDA GFTT/LK | diagnostic-only | 已修复非连续 ROI view 导致的 OpenCV CUDA texture 错误；扩 ROI 后仍 `93.6fps`、`0/555` 有效 | 不进入默认；排球 ROI 上当前跟踪点不可用 |
| CUDA Hough circle | diagnostic-only | `88.0fps`、`6/539` 有效 | 不进入默认；当前有效率和 GPU 争用都不合格 |
| XFeat TensorRT | `z_roi_neural_feature` | 96/top32 `97.9fps`、`3/627`；128/top32 `94.5fps`、`44/604`；160/top64 `93.5fps`、`32/601` | 后处理/NMS/匹配迁到 GPU 或重训前不准入 |
| SuperPoint TensorRT extractor | `z_roi_neural_feature` | top64 engine 可跑；128/160 为 `0` 有效，224/top64 为 `59.2fps`、`151/402` 有效 | 不准入；需优化后处理和匹配 |
| ALIKED TensorRT extractor | `z_roi_neural_feature` | NX 缺 `aliked_extractor_160_top64.engine` / `aliked_extractor_224_top64.engine` | 有可用 engine 后再进矩阵 |

### P2 已接入口但待实测后端

| 方向 | 建议形态 | 当前判断 |
|---|---|---|
| ROI ring/edge profile matcher | 复用 circle/edge/radial 信息，沿球边界或径向采样，做小范围极线匹配 | 初版自研 CUDA kernel 已实测无有效匹配；保留字段但默认关闭 |
...
| VPI CUDA Template Matching | `roi_vpi_template_match`，VPI CUDA stream wrapper + NCC score 小 ROI | 已修正 scratch 复用和 GPU peak reduce；此前有球测试 `99.4fps`、`0/645` 有效，不准入，需重测只看修正后耗时 |
| VPI CUDA Stereo Disparity | `roi_vpi_stereo_disparity`，小 ROI / `64` local disparity + confidence map | 已修正 scratch 复用和 `initial_disp ±32px` residual 搜索；此前有球测试 `100.1fps`、`0/632` 有效，不准入，需重测只看修正后耗时/有效率 |
| Fixstars libSGM | `roi_libsgm`，裁剪 ROI 或小分辨率双目图，4-path/subpixel SGM | 已接配置/diagnostic 入口；NX 当前未安装 libSGM，运行返回 unsupported，需先引入第三方库 |
| VPI Harris + Pyramidal LK | `roi_vpi_harris_lk` | 当前只是 availability stub；VPIArray/VPIPyramid 真实后端仍待实现 |
| OpenCV CUDA GFTT/Harris + SparsePyrLK | `roi_cuda_gftt_lk`，OpenCV CUDA 角点 + LK，小 ROI 小位移 | 真实 OpenCV CUDA 后端已实测；当前 0 有效且 FPS 不准入 |
| VPI ORB | `roi_vpi_orb` | 当前只是 availability stub；VPI descriptor matcher 真实后端仍待实现 |
| CUDA-SIFT 第三方实现 | `roi_cuda_sift`，CudaSift 等依赖，限制 ROI/top-k | 已接配置/diagnostic 入口；NX 当前未安装 CUDA-SIFT，运行返回 unsupported，需先引入第三方库 |
| BRISK/AKAZE GPU 路线 | 查找第三方 CUDA/VPI 替代或改用 ORB/GFTT+LK | 当前 OpenCV 没有项目已接入的 CUDA BRISK/AKAZE；算法概念保留 P2 调研，不跑 CPU 版当实时结论 |
| CUDA Canny/HoughCircles fallback refinement | `roi_cuda_hough_circle`，单侧 fallback 命中后，用边缘/圆检测细化中心和半径 | 真实 OpenCV CUDA Hough 已实测；当前 `88.0fps`、`6/539` 有效，不准入 |
