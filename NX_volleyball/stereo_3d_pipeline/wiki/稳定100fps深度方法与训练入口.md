...
2026-07-04 联合复测显示，`full12_default` 只有 `6.7fps`，`no_sgm_no_vpi` 可回到 `100.3fps`，但 worker `avg/p95/max=9.09/9.72/15.04ms`，仍有 `8` 次 stale 和 `62` 次 over deadline。因此当前测试配置改为: 去 SGM、去 VPI、去两个已确认错配的彩色 IoU/edge 候选；在这个基础上加入 XFeat TensorRT 主 CSV 候选和 OpenCV CUDA GFTT/LK diagnostic sidecar。

2026-07-04 14:23 NX 联合实测结果: 当前组合 `XFeat TensorRT 128/top32 + OpenCV CUDA GFTT/LK sidecar(stride=10)` 能运行并写出主 CSV、`.frames.csv` 和 `*.p2_diagnostic.csv`，但稳定段 ROI 输出只有 `93-95fps`，`Stage2_AsyncRoiWorker avg/p95/max=8.52/9.44/14.52ms`，`Stage2_AsyncRoiOverDeadline=62`。A/B 短测显示，关闭 XFeat 但保留 GFTT/LK sidecar 后稳定段回到 `99-100fps`，worker `avg/p95/max=4.51/5.07/10.31ms`；只保留 XFeat、关闭 GFTT/LK 时稳定段仍为 `94-98fps`，worker `avg/p95/max=8.51/9.08/13.49ms`。结论: GFTT/LK 低频 sidecar 可作为 diagnostic 观察项；XFeat 主 CSV 候选当前不满足 100fps 准入，正式 100fps 采集前应关闭或改为低频/条件触发。
...
这些方法已进入 `config/pipeline_dual_yolo_roi.yaml` 和 `config/pipeline_record_p0p1.yaml` 的主管线采集集合，但不能直接视为稳定主路径。训练读取原始候选字段或 diagnostic sidecar，用模型学习其偏置和可靠性；legacy `z_stereo/obj.z` 仍只允许 P0 几何/bbox 和退化 fallback。

当前测试态一共是 `13` 个主 CSV direct/fallback 深度字段 + `1` 个 diagnostic sidecar 字段: P0 direct 字段 `9` 个，包括 `z_bbox_center`、`z_bbox_left_edge`、`z_bbox_right_edge`、`z_circle_center`、`z_circle_left_edge`、`z_circle_right_edge`、`z_roi_edge_centroid`、`z_roi_radial_center`、`z_roi_edge_pair_center`；P1 主 CSV 字段 `3` 个，为 `z_roi_multi_point`、`z_roi_center_patch`、`z_roi_neural_feature`；单侧退化字段 `z_fallback_epipolar`；diagnostic sidecar 字段 `z_roi_opencv_cuda_gftt_lk`。按算法族粗分是 bbox、circle、ROI 几何、multi-point、center patch、XFeat、epipolar fallback、GFTT/LK 共 `8` 类。
...
| 主 CSV | XFeat TensorRT 128/top32 | `z_roi_neural_feature` | 组合 `93-95` | `52/2508` | `3.4287m` | `0.0024m` | 已接入主管线字段和真实 TensorRT extractor；当前主路径开销不满足 100fps，正式采集前应关闭或低频化 |
| diagnostic sidecar | OpenCV CUDA GFTT/LK | `mode=opencv_cuda_gftt_lk` -> `z_roi_opencv_cuda_gftt_lk` | 组合 `93-95`，无 XFeat A/B `99-100` | `114/251` | `3.4091m` | `0.0004m` | 低频 sidecar 可跑；不回写主 CSV，训练读取器会按 `frame_id + mode` 合并 sidecar 字段 |
...
- 训练采集: 100fps 准入版本应开 P0 + `z_roi_multi_point`、`z_roi_center_patch`，并可低频记录 `mode=opencv_cuda_gftt_lk` 到 `*.p2_diagnostic.csv`；`z_roi_neural_feature` 当前只能作为性能风险项单独测试，不能直接放入正式 100fps 采集。
- `roi_center_patch` 单算法历史测试覆盖有限，但 P0+P1 联合录制覆盖率已到 `97%` 左右；它仍有约 `+5cm` 系统偏置，不能作为主路径依赖。
- `*.p2_diagnostic.csv` 由 `--recording-out xxx.csv` 自动派生为 `xxx.p2_diagnostic.csv`；`trajectory_fusion/dataset.py` 会按 `frame_id + mode` 把 `opencv_cuda_gftt_lk` 合并为可训练候选字段。

## P2: 默认关闭的实验候选

当前冻结 P0 + 本页 P1 作为训练采集集合。P2 的含义是“有真实 GPU/TensorRT 路线或已接入字段，但尚未进入主管线 P1 采集集合”。P2 新增项仍必须单项开启、单项记录 `Stage2_AsyncRoiWorker`/有效率/深度偏置/长尾耗时，不能直接叠加进默认采集配置，也不能抢占 legacy `z_stereo/obj.z`。
...
判断优先级: `codex_p2_verify_20260704_104947` 是当前 P2 targeted 复核依据；`codex_p2_full_20260704_083048` 保留为广覆盖矩阵历史。debug / artifact run 只用于证明算法级点对或峰值位置，不用于 FPS 准入。2026-07-04 联合复测后，`patch_iou_color_edge_wide_search`、`iou_region_color_patch_wide_search` 因 artifact 显示系统性错配而退出默认配置；OpenCV CUDA StereoSGM、VPI Template、VPI ORB 因联合运行长尾或有效率问题退出默认配置。当前只新增 XFeat TensorRT 主字段和 OpenCV CUDA GFTT/LK sidecar，需下一轮 NX 联合长测确认组合开销。
...
| 6 | OpenCV CUDA Template patch9 | `z_roi_cuda_template_match`，`cv::cuda::TemplateMatching` | 10:49 targeted `97.2fps`；algo `3.24/4.21/61.01ms`；worker `3.34/4.40/61.26ms`；13:49 diagnostic `100.7fps` | `619/619`，median/MAD `3.2701/0.0007m`；13:49 `187/500` diagnostic valid | 全有效但 FPS 和长尾不准入；已可看模板峰值和 score patch | artifact<br><a href="assets/p2_20260704_update/opencv_cuda_template_score_patch.png"><img src="assets/p2_20260704_update/opencv_cuda_template_score_patch.png" width="220"></a> |
...
| 9 | XFeat TensorRT | `z_roi_neural_feature`，extractor-only TensorRT + 当前 C++ 后处理 | 10:49 targeted 128/top32 `93.2fps`；algo `4.14/5.35/11.88ms`；worker `4.27/5.57/12.81ms` | `317/579`，median/MAD `3.3155/0.0015m`；13:02 retest `67/586` | 已加入本轮默认组合主 CSV 字段；真实 TensorRT 路径可用，必须以新组合联测确认 100fps | artifact<br><a href="assets/p2_20260704_inline/neural_xfeat.png"><img src="assets/p2_20260704_inline/neural_xfeat.png" width="220"></a> |
| 10 | SuperPoint TensorRT | `z_roi_neural_feature`，fixed extractor TensorRT + 当前匹配 | 13:51 160/top64 `89.0fps`；algo `5.21/5.46/11.79ms`；worker `5.31/5.59/11.96ms`; 224/top64 `77.5fps` | 160/top64 `71/950`；224/top64 `1/827` | 已补真实点对 overlay，但 FPS/有效率不准入 | artifact<br><a href="assets/p2_20260704_update/neural_superpoint_160_top64.png"><img src="assets/p2_20260704_update/neural_superpoint_160_top64.png" width="220"></a> |
...
OpenCV CUDA GFTT/LK 不在 `Object3D` 主候选列表里，低频写入 `*.p2_diagnostic.csv`，由训练数据读取器按 `frame_id + mode=opencv_cuda_gftt_lk` 合并。

默认 `config/pipeline_dual_yolo_roi.yaml` 下，P0、`z_roi_multi_point`、`z_roi_center_patch` 和 `z_roi_neural_feature` 会写入主 CSV，OpenCV CUDA GFTT/LK 写入 sidecar。OpenCV ORB/Template/BM/SGM、VPI、ring-edge、CPU fallback 等其余 P2 默认关闭。实际允许参与 legacy `z_stereo` 兼容选择的候选为:
...
其中 `z_fallback_epipolar` 只在单侧漏检且极线 fallback 有效时排在该槽位；正常左右 YOLO 配对时同一槽位通常是 `z_circle_center`。`z_fallback` 仍作为兼容汇总字段写出。`z_roi_multi_point`、`z_roi_center_patch`、`z_roi_neural_feature` 和 GFTT/LK sidecar 会继续写入原始候选，但不会写入 legacy `z_stereo/obj.z`。模型训练和评估必须直接读取原始 P0/P1 字段。
...
- `z_roi_radial_center`
- `z_roi_edge_pair_center`
- P1 主 CSV 训练候选: `z_roi_multi_point`, `z_roi_center_patch`, `z_roi_neural_feature`
- P1 diagnostic sidecar 候选: `z_roi_opencv_cuda_gftt_lk`
...
2. 再加入 P1 的 `roi_multi_point`、`roi_center_patch`、XFeat 和 GFTT/LK sidecar，判断各自系统差是否稳定可校正。
...
z_roi_center_patch
z_roi_neural_feature
z_roi_opencv_cuda_gftt_lk   # from *.p2_diagnostic.csv
```

当前训练采集字段冻结为上述 P0+P1 集合；XFeat 写主 CSV，GFTT/LK 写 diagnostic sidecar 并由 `trajectory_fusion/dataset.py` 合并。彩色 IoU/edge、SGM、VPI 已从默认配置移除。P2 新算法可以有默认关闭字段和矩阵 case，但没有 NX isolated 和联合结果前不得加入主管线采集配置。
