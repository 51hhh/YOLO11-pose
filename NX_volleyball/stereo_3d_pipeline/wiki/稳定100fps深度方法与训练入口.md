...
当前 100fps 采集口径一共是 `12` 个主 CSV direct/fallback 深度字段 + `3` 个 P1 sidecar 字段。P0 direct 字段 `9` 个，包括 `z_bbox_center`、`z_bbox_left_edge`、`z_bbox_right_edge`、`z_circle_center`、`z_circle_left_edge`、`z_circle_right_edge`、`z_roi_edge_centroid`、`z_roi_radial_center`、`z_roi_edge_pair_center`；P1 主 CSV 字段 `2` 个，为 `z_roi_multi_point`、`z_roi_center_patch`；单侧退化字段 `z_fallback_epipolar`；P1 sidecar 字段为 `z_roi_cuda_template_match`、`z_roi_neural_xfeat`、`z_roi_neural_superpoint`。sidecar 通过 `*.p2_diagnostic.csv` 记录，不回写主 `Object3D`。
...
| P1 sidecar | 自研 CUDA Template/NCC | `mode=cuda_template` -> `z_roi_cuda_template_match` | 联合 `99.95` | `317/317` | `3.5032m` | `0.0020m` | `support=1` 的单点模板候选；不参与 legacy 输出 |
| P1 sidecar | XFeat TensorRT | `mode=neural_xfeat` -> `z_roi_neural_xfeat` | 联合 `99.95` | `185/317` | `3.4627m` | `0.0085m` | 有效率不足，保留为训练候选和后续优化目标 |
| P1 sidecar | SuperPoint TensorRT | `mode=neural_superpoint` -> `z_roi_neural_superpoint` | 联合 `99.95` | `317/317` | `3.4090m` | `0.0047m` | 覆盖率高，但相对 P0/P1 有系统差，需要模型学习偏置 |
...
- 训练采集: 100fps 准入版本应开 P0 + `z_roi_multi_point`、`z_roi_center_patch`，并记录 `mode=cuda_template`、`mode=neural_xfeat`、`mode=neural_superpoint` 到 `*.p2_diagnostic.csv`。
- `roi_center_patch` 单算法历史测试覆盖有限，但 P0+P1 联合录制覆盖率已到 `97%` 左右；它仍有约 `+5cm` 系统偏置，不能作为主路径依赖。
- `*.p2_diagnostic.csv` 由 `--recording-out xxx.csv` 自动派生为 `xxx.p2_diagnostic.csv`；`trajectory_fusion/dataset.py` 会按 `frame_id + mode` 把 NCC/XFeat/SuperPoint 合并为可训练候选字段。

## P2: 默认关闭的实验候选

当前冻结 P0 + 本页 P1 作为训练采集集合；NCC/XFeat/SuperPoint 已提升为 P1 sidecar 候选。其它 P2 新增项仍必须单项开启、单项记录 `Stage2_AsyncRoiWorker`/有效率/深度偏置/长尾耗时，不能直接叠加进默认采集配置，也不能抢占 legacy `z_stereo/obj.z`。
...
| 6 | 自研 CUDA Template/NCC | `z_roi_cuda_template_match`，gray patch NCC + GPU peak reduce | 2026-07-05 A/B `100.1fps`；algo `0.30/1.06/4.89ms`，p99 `1.20ms`；联合 run 主线 `99.95fps` | isolated `1372/1374`；联合 sidecar `317/317`，median/MAD `3.5032/0.0020m` | 已提升为 P1 sidecar 训练候选；单点 `support=1`，不参与 legacy `z_stereo` | artifact<br><a href="assets/cuda_template_ncc_20260705/frame_000002_valid.png"><img src="assets/cuda_template_ncc_20260705/frame_000002_valid.png" width="220"></a> |
...
| 9 | XFeat TensorRT | `z_roi_neural_xfeat`，extractor-only TensorRT + GPU postprocess | 10:49 targeted 128/top32 `93.2fps`；14:30 xfeat-only `98.3fps`；联合 run 主线 `99.95fps` | 联合 sidecar `185/317`，median/MAD `3.4627/0.0085m` | 已提升为 P1 sidecar 训练候选；有效率不足，仍是后续优化目标 | artifact<br><a href="assets/p2_20260704_inline/neural_xfeat.png"><img src="assets/p2_20260704_inline/neural_xfeat.png" width="220"></a> |
| 10 | SuperPoint TensorRT | `z_roi_neural_superpoint`，fixed extractor TensorRT + GPU mutual-NN | 13:51 160/top64 `89.0fps`；联合 run 主线 `99.95fps` | 联合 sidecar `317/317`，median/MAD `3.4090/0.0047m` | 已提升为 P1 sidecar 训练候选；覆盖率高但有系统差 | artifact<br><a href="assets/p2_20260704_update/neural_superpoint_160_top64.png"><img src="assets/p2_20260704_update/neural_superpoint_160_top64.png" width="220"></a> |
...
| 19 | ALIKED TensorRT | legacy neural direct extractor schema | skipped | NX 缺 `aliked_extractor_160_top64.engine` / `aliked_extractor_224_top64.engine` | 缺 engine；不参与准入 | 无图 |
...
默认 `config/pipeline_dual_yolo_roi.yaml` 下，P0、`z_roi_multi_point` 和 `z_roi_center_patch` 会写入主 CSV；NCC/XFeat/SuperPoint 作为 P1 sidecar 写入 `*.p2_diagnostic.csv` 并由训练读取器合并。OpenCV ORB、BM/SGM、VPI、ring-edge、GFTT/LK、CPU fallback 等其余 P2 默认关闭。实际允许参与 legacy `z_stereo` 兼容选择的候选为:
...
其中 `z_fallback_epipolar` 只在单侧漏检且极线 fallback 有效时排在该槽位；正常左右 YOLO 配对时同一槽位通常是 `z_circle_center`。`z_fallback` 仍作为兼容汇总字段写出。`z_roi_multi_point`、`z_roi_center_patch` 和 P1 sidecar 候选会继续写入原始候选，但不会写入 legacy `z_stereo/obj.z`。模型训练和评估必须直接读取原始 P0/P1 字段。
...
- P1 sidecar 候选: `z_roi_cuda_template_match`, `z_roi_neural_xfeat`, `z_roi_neural_superpoint`
...
z_roi_multi_point
z_roi_center_patch
z_roi_cuda_template_match     # from *.p2_diagnostic.csv mode=cuda_template
z_roi_neural_xfeat            # from *.p2_diagnostic.csv mode=neural_xfeat
z_roi_neural_superpoint       # from *.p2_diagnostic.csv mode=neural_superpoint
```

当前训练采集字段冻结为上述 P0+P1 集合；NCC/XFeat/SuperPoint 写 sidecar 并由 `trajectory_fusion/dataset.py` 合并。GFTT/LK、彩色 IoU/edge、SGM、VPI 已从默认配置移除。P2 新算法可以有默认关闭字段和矩阵 case，但没有 NX isolated 和联合结果前不得加入 P1 采集配置。
