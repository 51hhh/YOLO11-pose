...
当前默认配置结论: 联合运行已改为去 SGM、去 VPI，并关闭 artifact 显示错配的 `iou_region_color_patch` / `patch_iou_color_edge`。NCC、XFeat、SuperPoint 已提升为 P1 sidecar 训练候选；OpenCV CUDA GFTT/LK 默认关闭，仅保留 A/B 诊断。2026-07-04 14:27 旧组合 artifact 复测确认 XFeat 和 GFTT/LK 都能输出真实左右点对 overlay，但性能复测显示旧 XFeat 主路径使组合降到 `94.4fps`、xfeat-only 也只有 `98.3fps`，因此这些历史图只证明匹配可视化和字段接线正确，不代表当前 P1 sidecar 准入。

2026-07-05 补充: 自研 CUDA Template/NCC 单算法 isolated `100.1fps`、`1372/1374` 有效、algo `0.30/1.06/1.20/4.89ms`，当前替代 OpenCV CUDA TemplateMatching 作为默认 `z_roi_cuda_template_match` 后端。SuperPoint 160/top64 batch=2 + GPU mutual-NN 已接入 sidecar，stride=2 relaxed 为 `99.6fps`、`920/920` 有效。P0/P1+NCC+XFeat+SuperPoint 联合 run 主线 `99.95fps`，三算法由 sidecar 合并为 P1 训练候选。
...
NX run: /home/nvidia/trajectory_dataset/superpoint_b2_stride_20260705_084344/
NX debug: /home/nvidia/trajectory_dataset/superpoint_b2_debug_20260705_084510/
wiki/assets/superpoint_b2_20260705/
```
...
| SuperPoint TensorRT 160/top64 b2 | `20` | 2026-07-05 batch=2 + GPU mutual-NN sidecar；stride=2 relaxed `920/920` diagnostic valid，真实 neural keypoint matches | <img src="assets/superpoint_b2_20260705/frame_000002_00_neural_feature_ok_gpu_b2.png" width="320"> |
...
| SuperPoint 160/top64 b2 sidecar | `99.6fps`, `920/920` relaxed diagnostic valid | 已补真实 keypoint overlay；只能作为 stride=2 训练候选 sidecar |
...
| ALIKED-t16 DCN TensorRT | current `0/572`；gate-off `68/572`，support 中位 `2` | C | engine/plugin 已落地；zoom 显示点位大体在球面但点数太少，不准入 |
...
- [x] 自研 CUDA Template/NCC、OpenCV CUDA Template baseline 和 VPI Template 输出 score/peak artifact。
