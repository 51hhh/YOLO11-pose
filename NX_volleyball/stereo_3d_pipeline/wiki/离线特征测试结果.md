...
本页只记录离线 probe 和可视化结果，不代表 100fps 默认选用。NX 实时管线中的耗时、fps、CSV 有效率见 [实时特征算法矩阵](实时特征算法矩阵.md)，当前实际选用见 [稳定100fps深度方法与训练入口](稳定100fps深度方法与训练入口.md)，算法实现和实时后端边界见 [特征匹配实现](特征匹配实现.md)。深度相关页面总入口见 [深度算法导航](深度算法导航.md)。
