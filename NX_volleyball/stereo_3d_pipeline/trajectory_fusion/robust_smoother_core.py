"""Core robust smoother implementation for trajectory fusion."""
...
try:
    from .dataset import LegacySequence
    from .robust_smoother_measurements import initial_z, z_measurements
...
def smooth_sequence(sequence: LegacySequence, cfg: SmootherConfig) -> Tuple[List[Dict[str, float]], Dict[str, float]]:
    rows = sequence.rows
    first = rows[0]
    state = np.array([first["x"], first["y"], initial_z(first), 0.0, 0.0, 0.0], dtype=np.float64)
    ...
    for i, row in enumerate(rows):
        ...
        for z_value, z_var, _name in z_measurements(row, cfg):
