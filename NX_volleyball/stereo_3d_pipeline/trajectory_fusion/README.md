...
- `evaluate_reliability_smoother.py`: 将 ReliabilityNet 输出的 `bias/sigma/outlier_prob` 转成每路候选观测的偏置修正和方差，再交给 `robust_smoother` 物理平滑；这是当前推荐的模型接入评估入口。
- `evaluate_fusion.py`: 对原始/平滑输出做稳定性、known-distance bias/MAD、P0 median 和 pair gate 质量评估。
- `run_evaluation_suite.py`: 对单 CSV、多 CSV 或 manifest 批量生成 `check_dataset`、raw eval、robust smoother baseline；传 `--checkpoint` 时额外生成 direct ReliabilityNet 和 reliability smoother 对比结果。
