...
- `build_dataset_manifest.py`: 从采集目录生成 `dataset_manifest.yaml/json`，自动跳过 `.frames.csv`、`.p2_diagnostic.csv` 和 suite/sweep 派生 CSV，并按 `known_z` 片段生成 train/val/test split。
