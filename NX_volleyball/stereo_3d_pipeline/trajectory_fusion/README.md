...
- `run_evaluation_suite.py`: 对单 CSV、多 CSV 或 manifest 批量生成 `check_dataset`、候选一致性诊断、raw eval、robust smoother baseline；传 `--checkpoint` 时额外生成 direct ReliabilityNet 和 reliability smoother 对比结果。默认只用 `known_z` 评估 bias/MAD，不把它喂回 smoother。
