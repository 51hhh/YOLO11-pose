...
- `evaluate_calibrated_smoother.py`: 应用 `fit_method_calibration.py` 输出的每方法 bias/sigma，构造校准后候选观测，再交给 `robust_smoother` 物理平滑。
...
- `run_evaluation_suite.py`: 对单 CSV、多 CSV 或 manifest 批量生成 `check_dataset`、候选一致性诊断、raw eval、robust smoother baseline 和 `robust_rts_smooth` 离线 RTS baseline；传 `--calibration` 时额外生成每方法校准 smoother 对比，传 `--checkpoint` 时额外生成 direct ReliabilityNet 和 reliability smoother 对比结果。默认只用 `known_z` 评估 bias/MAD，不把它喂回 smoother。
- `summarize_evaluation_suite.py`: 汇总 suite 输出目录，把 raw/robust/RTS/direct/reliability smoother 的核心指标写成单个 `suite_metrics.csv`。
- `rank_sweep_metrics.py`: 对 `sweep_metrics.csv` 按 known-distance bias/MAD、稳定性和峰峰值排序，输出 `sweep_ranking.csv`；默认 `--split auto`，有 `val` split 时只用 heldout 指标排名。`--variant all` 可把 `calibrated_smoother`、`robust_smooth`、`robust_rts_smooth` 和 ReliabilityNet variants 放在同一张 variant ranking 里对比。
- `run_reliability_sweep.py`: 批量训练多组 ReliabilityNet 配置，并对每个 checkpoint 自动运行 suite、汇总指标和排名；可传 `--calibration` 把每方法校准 baseline 一起写进每组 suite 指标，适合明天多段 known-distance/heldout 数据做模型选择。
...
`robust_rts_smooth` 是 suite 默认生成的离线全序列 RTS 对照。它可以利用未来帧降低抖动，适合明天 known-distance 数据到位后做伪真值候选和上限参考；如果要只看实时因果可部署路径，使用 `robust_smooth` 或在 suite 上加 `--skip-rts-smoother`。
