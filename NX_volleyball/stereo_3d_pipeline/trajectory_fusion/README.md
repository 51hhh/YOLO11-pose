...
- `evaluate_reliability_smoother.py`: 将 ReliabilityNet 输出的 `bias/sigma/outlier_prob` 转成每路候选观测的偏置修正和方差，再交给 `robust_smoother` 物理平滑；这是当前推荐的模型接入评估入口。默认不把 `known_z` 作为 smoother update，避免评估标签泄漏。`--rts` 只用于离线全序列对照。
...
- `run_evaluation_suite.py`: 对单 CSV、多 CSV 或 manifest 批量生成 `check_dataset`、候选一致性诊断、raw eval、robust smoother baseline 和 `robust_rts_smooth` 离线 RTS baseline；传 `--calibration` 时额外生成 `calibrated_smoother` / `calibrated_rts_smoother`，传 `--checkpoint` 时额外生成 direct ReliabilityNet、`reliability_smoother` / `reliability_rts_smoother` 对比结果。默认只用 `known_z` 评估 bias/MAD，不把它喂回 smoother。
- `summarize_evaluation_suite.py`: 汇总 suite 输出目录，把 raw/robust/RTS/calibrated/direct/reliability smoother 的核心指标写成单个 `suite_metrics.csv`，其中包含 `ballistic_residual_rms_mps2`、`accel_z_rms_mps2`、`speed_rms_mps` 等动态诊断列。
...
`robust_rts_smooth`、`calibrated_rts_smoother` 和 `reliability_rts_smoother` 是 suite 默认生成的离线全序列 RTS 对照。它们可以利用未来帧降低抖动，适合明天 known-distance 数据到位后做伪真值候选和上限参考；如果要只看实时因果可部署路径，使用不带 `_rts_` 的 variant 或在 suite 上加 `--skip-rts-smoother`。
