...
- `run_dataset_workflow.py`: 采集后的一键入口，串起 manifest 生成/预检、每方法 known-distance calibration、baseline suite 和可选 ReliabilityNet sweep，并写 `workflow_summary.json`、`workflow_report.json`、`workflow_report.md`。默认不把 `known_z` 喂给 smoother；需要标签辅助伪真值时才显式加 `--use-static-known-z`。
