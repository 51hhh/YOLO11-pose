...
bbox_center disparity (+ circle fallback/consistency)
  -> d0 反投影到 3D
  -> Student-t 鲁棒 EKF（重力 + 二次阻力, Cd=0.10）
  -> RK4 rollout 到地面
  -> 可选 TinyGRU 只修正落点平面残差
```

代码位置:

- `landing_pipeline/`: 可部署因果实现
  - `observation.py`: bbox 主观测 + d0 反投影；circle 仅 fallback/一致性
  - `ekf.py`: Student-t EKF
  - `physics.py`: RK4 落点
  - `residual_gru.py`: 可选 TinyGRU 残差（无 torch/权重时自动退回纯物理）
  - `predictor.py`: `LandingPipeline` 流式 API
  - `run_csv.py`: 离线 CSV 回放入口
- `configs/landing_pipeline_bbox_ekf.json`: 默认参数
- `tests/test_landing_pipeline.py`: 合成轨迹单测
...
pipe = LandingPipeline.from_default(
    config_path="trajectory_fusion/configs/landing_pipeline_bbox_ekf.json",
    enable_residual=False,  # 无 torch 时建议先关
)
for row in csv_dict_reader:
    out = pipe.update_row(row)
    if out is not None:
        print(out.landing, out.time_to_land, out.source)
```

C++ 实时接入:

- `src/fusion/trajectory_predictor.*` 已切换为同一主路径（Student-t EKF + RK4）
- 配置见 `config/pipeline_dual_yolo_roi.yaml` 的 `prediction:`
- 说明见 `wiki/实时落点预测EKF.md`
- TinyGRU 残差仍仅 Python 可选，未进 C++ 实时依赖

设计约束:

- 主观测固定 `bbox_center`；不要把多路 `z_*` 当独立观测同时喂 EKF。
- 深度必须 `Z = fB / (disparity - d0)`，不要直接读 raw `z_*`。
- TinyGRU 只修落点残差，可关、可限幅、可回退。
...
- `audit_training_inputs.py`: 复用 `build_legacy_arrays()` 审计 ReliabilityNet 实际输入，输出每个候选方法的有效率、每个特征的非零率，并检查 legacy `z_stereo/z` 和在线状态字段是否泄漏回训练输入。
- `fit_method_calibration.py`: 从 `train` split 的 known-distance 静态片段拟合每种测距方法的 median bias 和 sigma，生成非神经 calibration JSON。
- `fit_measurement_noise_calibration.py`: 从 known-distance 静态片段拟合可部署观测噪声表；先按 calibration + d0 重反投影各路 disparity，再按方法、距离桶、径向桶和图像网格估计 `bias_median/sigma/outlier_rate/valid_rate`。输出用于 EKF/UKF 的观测修正和 `Rz`，不是深度字段投票器。
...
- `run_dataset_workflow.py`: 采集后的一键入口，串起 manifest 生成/预检、训练输入审计、每方法 known-distance calibration、baseline suite 和可选 ReliabilityNet sweep，并写 `workflow_summary.json`、`workflow_report.json`、`workflow_report.md`。生成 manifest 时可加 `--stratify-known-z`；默认不把 `known_z` 喂给 smoother，需要标签辅助伪真值时才显式加 `--use-static-known-z`。
- `run_workflow_matrix.py`: 对一个采集目录运行标准 workflow 矩阵，包括 `known_stratified`、每个 known_z 桶的 leave-one-distance-out holdout，以及可选 dynamic regularization，并自动生成 workflow 横向比较表。
...
- `compare_workflows.py`: 横向比较多个 workflow 目录，输出 readiness、top selection、best variant、候选一致性摘要和 warning 汇总，适合多距离/leave-one-distance/dynamic 多组实验后快速筛选。
- `configs/`: `run_dataset_workflow.py --configs` / `run_reliability_sweep.py --configs` 使用的固定 JSON 配置；`sweep_smoke.json` 用于链路冒烟，`sweep_known_distance_selection.json` 用于首轮 known-distance 模型选择，`sweep_dynamic_regularization.json` 用于动态片段第二轮探索。`measurement_noise_20260709_static.json` 是 2026-07-09 静态 known-distance 观测噪声标定输入，排除了坏段 `141742` 和标签疑似的 13m 桶。

## Measurement Noise Calibration

2026-07-09 这批静态米尺数据的可复现标定命令：

```bash
python3 NX_volleyball/stereo_3d_pipeline/trajectory_fusion/fit_measurement_noise_calibration.py \
  NX_volleyball/stereo_3d_pipeline/trajectory_fusion/configs/measurement_noise_20260709_static.json \
  --known-z-labels NX_volleyball/stereo_3d_pipeline/trajectory_fusion/configs/measurement_noise_20260709_static.json \
  --calib NX_volleyball/calibration/stereo_calib.yaml \
  --offset-fit NX_volleyball/stereo_3d_pipeline/config/disparity_offset_fit_20260709.json \
  --methods p0p1_ncc_xfeat \
  -o NX_volleyball/stereo_3d_pipeline/trajectory_fusion/measurement_noise_calibration_20260709.json
```

输出 JSON 的在线含义：

```text
z_corr = z_d0_reprojected - bias
Rz = sigma^2
```

查表顺序建议先用 `by_distance_grid` 或 `by_distance_radial` 的 `bias_shrunk/sigma_shrunk`，不足或低置信时退回 `by_distance`，最后退回 `global`。`calibrated_confidence` 低、`outlier_rate` 高的桶不应作为强观测；进入 Student-t 鲁棒更新或直接降权。对 3D EKF 更新，把深度噪声沿反投影雅可比传播：

```text
Jz = [(u - cx) / fx, (v - cy) / fy, 1]^T
R_xyz_depth = sigma_z^2 * Jz * Jz^T
```

这张表解决的是“同一深度方法在不同距离/画面位置的系统偏置和观测方差”。它不负责重新估计 d0，也不应该把强相关的 P0 候选当作独立传感器重复收缩协方差。
