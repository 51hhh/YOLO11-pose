...
- `legacy`: 旧 CSV 列，最轻量。
- `depth_candidates`: 追加 `class_id,z_bbox_center,z_bbox_left_edge,z_bbox_right_edge,z_circle_center,z_circle_left_edge,z_circle_right_edge,z_roi_edge_centroid,z_roi_radial_center,z_roi_edge_pair_center,z_roi_corner_points,z_roi_texture_points,z_roi_binary_points,z_roi_orb_points,z_roi_brisk_points,z_roi_akaze_points,z_roi_center_patch,z_roi_multi_point,z_fallback,z_fallback_template,z_fallback_feature_points` 及对应 `disparity_*`、稀疏特征 `support/std/confidence` 字段，同时保留 `z_yolo_bbox_pair,z_circle,z_subpixel` 旧兼容别名。
...
`roi_orb_points/roi_brisk_points/roi_akaze_points` 使用 OpenCV `features2d` 在左右 bbox ROI 内检测二值描述子，并用 Hamming KNN、ratio test、极线 y、正视差、动态视差门限和 MAD 一致性过滤。它们默认关闭，避免在 100fps 实机路径中隐式增加 CPU 开销。XFeat 需要 PyTorch/ONNX/TensorRT 模型和 C++ 推理封装，当前工程只记录为后续可接入方向，未作为可运行深度模式启用。

仍然缺少训练可靠性模型最关键的一部分质量特征，例如被拒绝候选、每个失败原因、完整左右检测列表和相机曝光/增益逐帧值。因此：

- 新录制数据可以开始分析 bbox/circle/subpixel/fallback 的互补关系；训练脚本会自动读取候选深度、质量特征、同步差值、左右 bbox 和圆心几何。
- 旧 CSV 仍然只能训练/评估很弱的可靠性模型。
- 真正要让模型学会“哪种深度在当前帧可信”，还需要继续按 `schema.md` 补全在线记录器。
