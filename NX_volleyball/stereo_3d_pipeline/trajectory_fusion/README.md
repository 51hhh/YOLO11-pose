...
- `robust_smoother.py`: 候选深度驱动的鲁棒物理平滑 baseline；默认不使用 legacy `z_stereo/z` 作为观测，会先按 mono/bbox/circle/ROI/fallback 等相关方法组聚合再更新，避免同源候选被当作独立观测重复收缩协方差。
- `models.py`: 可靠性网络骨架，只输出噪声/偏差/离群概率，不直接输出最终轨迹。
- `losses.py`: 自监督观测似然、物理约束和正则项。
- `train_reliability.py`: 可靠性训练脚手架，支持 `metadata.yaml` 中的 `known_z`, `known_z_min/max`, `static` 弱标签。
...
- 新录制数据可以开始分析 bbox/circle/subpixel/fallback 的互补关系；训练脚本会读取候选深度、候选集合统计、质量特征、同步差值、左右 bbox、圆心几何和 selected pair gate 字段。`METHOD_COLUMNS` 不使用旧在线 `z_stereo/z`，训练特征也不使用旧在线 `x/y/z/vx/vy/vz`。
