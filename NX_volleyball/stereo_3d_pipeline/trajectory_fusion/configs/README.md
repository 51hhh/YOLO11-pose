...
Configs may include `methods`, either as a preset string or comma-separated method names. Supported presets include `p0`, `p0p1`, `p0p1_ncc`, `p0p1_xfeat`, and `p0p1_ncc_xfeat`. The allowlist is applied consistently to ReliabilityNet training, direct consensus, robust smoother, calibrated smoother, and ReliabilityNet smoother evaluation. The formal known-distance/dynamic configs should stay pinned to `p0p1_ncc_xfeat`; use `sweep_method_ablation.json` when comparing method subsets.
