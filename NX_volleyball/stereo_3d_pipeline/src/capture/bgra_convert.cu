...
// BGRA → BGR (strip alpha channel)
__global__ void bgra2bgrKernel(
    const uint8_t* __restrict__ src, int srcPitch,
    uint8_t* __restrict__ dst, int dstPitch,
    int width, int height)
{
    ...
    const uint8_t* pixel = src + y * srcPitch + x * 4;
    uint8_t* out = dst + y * dstPitch + x * 3;
    out[0] = pixel[0];  // B
    out[1] = pixel[1];  // G
    out[2] = pixel[2];  // R
}

extern "C" void launchBGRA2BGR(
    const uint8_t* src, int srcPitch,
    uint8_t* dst, int dstPitch,
    int width, int height,
    cudaStream_t stream)
{
    dim3 block(32, 8);
    dim3 grid((width + block.x - 1) / block.x, (height + block.y - 1) / block.y);
    bgra2bgrKernel<<<grid, block, 0, stream>>>(src, srcPitch, dst, dstPitch, width, height);
}
