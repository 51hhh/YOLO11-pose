"""Gravity EKF + One-Euro variants for Round 3 exploration.

Variant A: EKF velocity drives One-Euro cutoff (instead of position derivative)
Variant B: Innovation-adaptive min_cutoff (responds faster to model mismatch)
Variant C: AEKF (adaptive R) + One-Euro post-smoothing
"""

import numpy as np
from .base import FilterBase, FilterState


class GravityOneEuroV2(FilterBase):
    """Variant A: Use EKF estimated velocity to drive One-Euro cutoff.
    
    Instead of computing dx/dt from position changes (lagged),
    use EKF's internal velocity estimate (instant, physics-informed).
    """

    def __init__(self, sigma_a=5.0, R_base=0.3, noise_exponent=2.85,
                 innovation_gate=25.0, min_cutoff=0.15, beta=0.0005,
                 focal=727.0, gravity=9.81, gravity_vec=None, **kwargs):
        super().__init__(**kwargs)
        self.sigma_a = sigma_a
        self.R_base = R_base
        self.noise_exponent = noise_exponent
        self.innovation_gate = innovation_gate
        self.min_cutoff = min_cutoff
        self.beta = beta
        self.focal = focal
        self.g_vec = np.array(gravity_vec if gravity_vec else [0, 9.81, 0])
        self.reset()

    def reset(self):
        self.x = np.zeros(6)
        self.P = np.eye(6) * 1.0
        self.P[3:, 3:] *= 10.0
        self.smooth_pos = np.zeros(3)
        self._initialized = False

    def _alpha(self, cutoff, dt):
        tau = 1.0 / (2 * np.pi * cutoff)
        return 1.0 / (1.0 + tau / dt)

    def predict(self, dt: float):
        self.x[3] += self.g_vec[0] * dt
        self.x[4] += self.g_vec[1] * dt
        self.x[5] += self.g_vec[2] * dt
        self.x[:3] += self.x[3:6] * dt

        F = np.eye(6)
        F[0, 3] = dt; F[1, 4] = dt; F[2, 5] = dt
        q = self.sigma_a ** 2
        Q = np.zeros((6, 6))
        for i in range(3):
            Q[i, i] = q * dt**4 / 4
            Q[i, i+3] = q * dt**3 / 2
            Q[i+3, i] = q * dt**3 / 2
            Q[i+3, i+3] = q * dt**2
        self.P = F @ self.P @ F.T + Q
        self._last_dt = dt

    def update(self, obs_x, obs_y, obs_z) -> FilterState:
        if not self._initialized:
            self.x[:3] = [obs_x, obs_y, obs_z]
            self.smooth_pos = self.x[:3].copy()
            self._initialized = True
            self._last_dt = 1.0 / 60.0
            return self.get_state()

        H = np.zeros((3, 6))
        H[0, 0] = H[1, 1] = H[2, 2] = 1.0

        z_c = max(1.0, abs(obs_z))
        Rz = self.R_base * (z_c ** self.noise_exponent)
        Rxy = Rz * z_c**2 / self.focal**2 + 0.001
        R = np.diag([Rxy, Rxy, Rz])

        y = np.array([obs_x, obs_y, obs_z]) - H @ self.x
        S = H @ self.P @ H.T + R
        S_inv = np.linalg.inv(S)
        maha2 = y @ S_inv @ y

        if maha2 <= self.innovation_gate:
            K = self.P @ H.T @ S_inv
            self.x += K @ y
            IKH = np.eye(6) - K @ H
            self.P = IKH @ self.P @ IKH.T + K @ R @ K.T

        # One-Euro using EKF velocity (key difference from V1)
        dt = getattr(self, '_last_dt', 1.0/60.0)
        ekf_pos = self.x[:3].copy()
        ekf_vel = self.x[3:6].copy()  # Use EKF velocity directly!

        speed = np.abs(ekf_vel)
        cutoff = self.min_cutoff + self.beta * speed
        a = np.array([self._alpha(c, dt) for c in cutoff])
        self.smooth_pos = a * ekf_pos + (1 - a) * self.smooth_pos

        return self.get_state()

    def get_state(self) -> FilterState:
        return FilterState(
            x=self.smooth_pos[0], y=self.smooth_pos[1], z=self.smooth_pos[2],
            vx=self.x[3], vy=self.x[4], vz=self.x[5],
        )


class GravityOneEuroAdaptive(FilterBase):
    """Variant B: Innovation-adaptive min_cutoff.
    
    When Mahalanobis distance is large, increase cutoff for faster response.
    When innovation is small (steady state), use low cutoff for smoothing.
    """

    def __init__(self, sigma_a=5.0, R_base=0.3, noise_exponent=2.85,
                 innovation_gate=25.0, min_cutoff=0.15, max_cutoff=5.0,
                 beta=0.0005, innov_scale=0.5,
                 focal=727.0, gravity=9.81, gravity_vec=None, **kwargs):
        super().__init__(**kwargs)
        self.sigma_a = sigma_a
        self.R_base = R_base
        self.noise_exponent = noise_exponent
        self.innovation_gate = innovation_gate
        self.min_cutoff = min_cutoff
        self.max_cutoff = max_cutoff
        self.beta = beta
        self.innov_scale = innov_scale
        self.focal = focal
        self.g_vec = np.array(gravity_vec if gravity_vec else [0, 9.81, 0])
        self.reset()

    def reset(self):
        self.x = np.zeros(6)
        self.P = np.eye(6) * 1.0
        self.P[3:, 3:] *= 10.0
        self.smooth_pos = np.zeros(3)
        self.smooth_vel = np.zeros(3)
        self._last_maha = 0.0
        self._initialized = False

    def _alpha(self, cutoff, dt):
        tau = 1.0 / (2 * np.pi * cutoff)
        return 1.0 / (1.0 + tau / dt)

    def predict(self, dt: float):
        self.x[3] += self.g_vec[0] * dt
        self.x[4] += self.g_vec[1] * dt
        self.x[5] += self.g_vec[2] * dt
        self.x[:3] += self.x[3:6] * dt

        F = np.eye(6)
        F[0, 3] = dt; F[1, 4] = dt; F[2, 5] = dt
        q = self.sigma_a ** 2
        Q = np.zeros((6, 6))
        for i in range(3):
            Q[i, i] = q * dt**4 / 4
            Q[i, i+3] = q * dt**3 / 2
            Q[i+3, i] = q * dt**3 / 2
            Q[i+3, i+3] = q * dt**2
        self.P = F @ self.P @ F.T + Q
        self._last_dt = dt

    def update(self, obs_x, obs_y, obs_z) -> FilterState:
        if not self._initialized:
            self.x[:3] = [obs_x, obs_y, obs_z]
            self.smooth_pos = self.x[:3].copy()
            self._initialized = True
            self._last_dt = 1.0 / 60.0
            return self.get_state()

        H = np.zeros((3, 6))
        H[0, 0] = H[1, 1] = H[2, 2] = 1.0

        z_c = max(1.0, abs(obs_z))
        Rz = self.R_base * (z_c ** self.noise_exponent)
        Rxy = Rz * z_c**2 / self.focal**2 + 0.001
        R = np.diag([Rxy, Rxy, Rz])

        y = np.array([obs_x, obs_y, obs_z]) - H @ self.x
        S = H @ self.P @ H.T + R
        S_inv = np.linalg.inv(S)
        maha2 = y @ S_inv @ y
        self._last_maha = maha2

        if maha2 <= self.innovation_gate:
            K = self.P @ H.T @ S_inv
            self.x += K @ y
            IKH = np.eye(6) - K @ H
            self.P = IKH @ self.P @ IKH.T + K @ R @ K.T

        # Adaptive cutoff: higher when innovation is large
        dt = getattr(self, '_last_dt', 1.0/60.0)
        ekf_pos = self.x[:3].copy()

        # Smooth maha for cutoff adaptation
        innov_factor = min(self.max_cutoff, self.min_cutoff + self.innov_scale * np.sqrt(maha2))

        # Also use speed
        d_cutoff = 1.0
        a_d = self._alpha(d_cutoff, dt)
        vel = (ekf_pos - self.smooth_pos) / dt
        self.smooth_vel = a_d * vel + (1 - a_d) * self.smooth_vel
        speed = np.abs(self.smooth_vel)

        cutoff = innov_factor + self.beta * speed
        a = np.array([self._alpha(c, dt) for c in cutoff])
        self.smooth_pos = a * ekf_pos + (1 - a) * self.smooth_pos

        return self.get_state()

    def get_state(self) -> FilterState:
        return FilterState(
            x=self.smooth_pos[0], y=self.smooth_pos[1], z=self.smooth_pos[2],
            vx=self.x[3], vy=self.x[4], vz=self.x[5],
        )


class AEKFOneEuro(FilterBase):
    """Variant C: Adaptive EKF (online R estimation) + One-Euro post-smoother."""

    def __init__(self, sigma_a=5.0, R_base=0.3, noise_exponent=2.85,
                 innovation_gate=25.0, window_size=20,
                 min_cutoff=0.15, beta=0.0005,
                 focal=727.0, gravity=9.81, gravity_vec=None, **kwargs):
        super().__init__(**kwargs)
        self.sigma_a = sigma_a
        self.R_base = R_base
        self.noise_exponent = noise_exponent
        self.innovation_gate = innovation_gate
        self.window_size = window_size
        self.min_cutoff = min_cutoff
        self.beta = beta
        self.focal = focal
        self.g_vec = np.array(gravity_vec if gravity_vec else [0, 9.81, 0])
        self.reset()

    def reset(self):
        self.x = np.zeros(6)
        self.P = np.eye(6) * 1.0
        self.P[3:, 3:] *= 10.0
        self.smooth_pos = np.zeros(3)
        self.smooth_vel = np.zeros(3)
        self.innovations = []
        self.S_history = []
        self.R_scale = 1.0
        self._initialized = False

    def _alpha(self, cutoff, dt):
        tau = 1.0 / (2 * np.pi * cutoff)
        return 1.0 / (1.0 + tau / dt)

    def _adapt_R(self):
        if len(self.innovations) < self.window_size:
            return
        innov = np.array(self.innovations[-self.window_size:])
        C_innov = np.mean([np.outer(v, v) for v in innov], axis=0)
        S_mean = np.mean(self.S_history[-self.window_size:], axis=0)
        ratio = np.trace(C_innov) / (np.trace(S_mean) + 1e-10)
        self.R_scale = 0.9 * self.R_scale + 0.1 * np.clip(ratio, 0.1, 10.0)

    def predict(self, dt: float):
        self.x[3] += self.g_vec[0] * dt
        self.x[4] += self.g_vec[1] * dt
        self.x[5] += self.g_vec[2] * dt
        self.x[:3] += self.x[3:6] * dt

        F = np.eye(6)
        F[0, 3] = dt; F[1, 4] = dt; F[2, 5] = dt
        q = self.sigma_a ** 2
        Q = np.zeros((6, 6))
        for i in range(3):
            Q[i, i] = q * dt**4 / 4
            Q[i, i+3] = q * dt**3 / 2
            Q[i+3, i] = q * dt**3 / 2
            Q[i+3, i+3] = q * dt**2
        self.P = F @ self.P @ F.T + Q
        self._last_dt = dt

    def update(self, obs_x, obs_y, obs_z) -> FilterState:
        if not self._initialized:
            self.x[:3] = [obs_x, obs_y, obs_z]
            self.smooth_pos = self.x[:3].copy()
            self._initialized = True
            self._last_dt = 1.0 / 60.0
            return self.get_state()

        H = np.zeros((3, 6))
        H[0, 0] = H[1, 1] = H[2, 2] = 1.0

        z_c = max(1.0, abs(obs_z))
        Rz = self.R_base * (z_c ** self.noise_exponent) * self.R_scale
        Rxy = Rz * z_c**2 / self.focal**2 + 0.001
        R = np.diag([Rxy, Rxy, Rz])

        y = np.array([obs_x, obs_y, obs_z]) - H @ self.x
        S = H @ self.P @ H.T + R

        self.innovations.append(y.copy())
        self.S_history.append(S.copy())
        self._adapt_R()

        S_inv = np.linalg.inv(S)
        maha2 = y @ S_inv @ y

        if maha2 <= self.innovation_gate:
            K = self.P @ H.T @ S_inv
            self.x += K @ y
            IKH = np.eye(6) - K @ H
            self.P = IKH @ self.P @ IKH.T + K @ R @ K.T

        # One-Euro post-smoothing
        dt = getattr(self, '_last_dt', 1.0/60.0)
        ekf_pos = self.x[:3].copy()

        d_cutoff = 1.0
        a_d = self._alpha(d_cutoff, dt)
        vel = (ekf_pos - self.smooth_pos) / dt
        self.smooth_vel = a_d * vel + (1 - a_d) * self.smooth_vel

        speed = np.abs(self.smooth_vel)
        cutoff = self.min_cutoff + self.beta * speed
        a = np.array([self._alpha(c, dt) for c in cutoff])
        self.smooth_pos = a * ekf_pos + (1 - a) * self.smooth_pos

        return self.get_state()

    def get_state(self) -> FilterState:
        return FilterState(
            x=self.smooth_pos[0], y=self.smooth_pos[1], z=self.smooth_pos[2],
            vx=self.x[3], vy=self.x[4], vz=self.x[5],
        )
